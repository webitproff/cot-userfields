...
function userfields_get_user_fields($user_id) {
    ...
    foreach ($fields as &$field) {
        $field['value'] = userfields_format_value($field['field_type'], $field['value'], true); // decode if needed
    }
...
function userfields_build_input($name, $extrafield, $data, $filter_mode = false) {
    ...
    switch ($type) {
        ...
        case 'double':
            ...
            return cot_inputbox('text', $name, $data);

        case 'textarea':
            return cot_textarea($name, $data, 4, 56);
...
function userfields_format_value_for_save($field_id, $value) {
	...
	if ($type === 'checklistbox' && is_array($value)) {
		// Удаляем пустые значения и 'nullval'
		...
		if (is_array($value)) {
			// Если значение приходит как массив (например, от cot_selectbox_date)
			$date = cot_mktime($value['year'], $value['month'], $value['day'], $value['hour'], $value['minute'], 0);
			return (int)$date;
		}
		return (int)$value; // Для обратной совместимости, если значение уже timestamp
	}
