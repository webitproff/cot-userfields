...
Cot::$db->registerTable('userfield_values');
// регистрируем нашу таблицу
Cot::$db->registerTable('userfield_types');

// бывает и так, чтоб не гадать
$db_userfield_values = $db_x . 'userfield_values';
$db_userfield_types = $db_x . 'userfield_types';


// заголовок браузера на странице администрирования плагина https://supersite.com/admin/other?p=userfields
$adminTitle = Cot::$L['userfields_adminTitle'] ?? '';

// текст справки по администрированию плагина, если такая строка есть в файле локализации
$adminHelp = Cot::$L['userfields_adminHelp'] ?? 'help text not writed yet'; 

// присваиваем шаблону имя части или локации расширения
$tpl_PartExt = 'admin';

// Загружаем шаблон для админки плагина userfields
$mskin = cot_tplfile(['userfields', $tpl_PartExt], 'plug');
